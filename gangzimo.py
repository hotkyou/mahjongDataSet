...
def gangzimo(self, i):
    ...
    #if self.reach[i[z]["l"]] != 1: #これがいるか怪しい csvで手牌が14になってるかチェック
        ...
        if self.todo == 3:
            ...
            if indexes != []: #プレイヤーがポンしている時
                if self.dorall.index((i[z]["p"])) in indexes:
                    ...
                    self.csvdata = data
                    #カカンcsv処理終了
                elif self.dorall.index((i[z]["p"])) in [9, 19, 29]:
                    if self.naki[i[z]["l"]][4] == 3 or self.naki[i[z]["l"]][14] == 3 or self.naki[i[z]["l"]][24] == 3:
                        #カカンcsv処理
                        data = []
                        data += self.tehaiok[i[z]["l"]] #手牌
                        for j in range(len(self.reach)): #リーチ自分から見て
